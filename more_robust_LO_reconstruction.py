...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 18 18:58:10 2025
...
def laing_obrien_reconstruct(spp, tpp, n, eps=1e-18):
    ...
    # Delta for phase angle (looks good)
    def delta(g, h, j, k):
    ...
    for g in range(n):
        for h in range(n):
            if g == 0:
                ...
                arg = np.clip(arg, -1.0, 1.0)            # <<< NEW: keep inside arccos domain
    ...
    t_row = np.sqrt(np.clip(np.real(t_squared_row), 0.0, None))     # <<< NEW
    t_col = np.sqrt(np.clip(np.real(t_squared_column), 0.0, None))  # <<< NEW
    ...
    for g in range(1, n):
        for h in range(1, n):
            base = M2[0, 0]
            # If the absolute reference is tiny, nudge it to avoid blow-ups (noise can do this)
            if abs(base) < eps:
                base = eps
    ...
    # --- Polar decomposition projection to the nearest unitary ---
    # M2 = U Σ V†  =>  nearest unitary = U V†
    try:
        Uu, _, Vh = np.linalg.svd(M2, full_matrices=False)
        M2 = Uu @ Vh
    except np.linalg.LinAlgError:
        # Extremely rare; if SVD fails, just return the unprojected M2
        pass
...
if __name__ == "__main__":
    ...
    spp, tpp = mds.compute_transition_probabilities(U) # compute measurement data
    M2 = laing_obrien_reconstruct(spp, tpp, N) # reconstruct the matrix from noisy measurement data
